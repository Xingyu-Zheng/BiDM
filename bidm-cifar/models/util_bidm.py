...
class _BiAct(nn.Module):
    def __init__(self):
        super(_BiAct, self).__init__()
        self.init = False
        
    def forward(self, input):
        if not self.init:
            self.channel_threshold = torch.nn.Parameter(torch.zeros(1, input.shape[1], 1), requires_grad=True).to(input.device)
            self.init = True
...
class BNNConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, bias=False, dilation=0, transposed=False, output_padding=None, groups=1, precision='bnn'):
        ...
        self.fliter_k = nn.Conv2d(1, 1, self.kernel_size, stride=self.stride, padding=self.padding)
        self.fliter_k.weight.data = torch.full(self.fliter_k.weight.shape, 1/self.kernel_size/self.kernel_size)
        if self.in_channels != self.out_channels:
            self.shortcut = nn.Conv2d(self.in_channels, self.out_channels, kernel_size=1, stride=self.stride, padding=0)
        self.shortcut_scale = torch.nn.Parameter(torch.ones(1) * 0.0, requires_grad=True)
    ...
    def forward(self, x, bnn_mode='bnn'):
        ...
        if not self.init_fliter_k:
            self.fliter_k.weight.data = torch.full(self.fliter_k.weight.shape, 1/self.kernel_size/self.kernel_size).cuda()
        ...
        return y + shortcut * self.shortcut_scale
...
class BNNConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, bias=False, dilation=0, transposed=False, output_padding=None, groups=1, precision='bnn', order=2):
        ...
        self.bias = nn.Parameter(torch.rand(out_channels) * 0.001, requires_grad=True)
        self.channel_threshold = nn.Parameter(torch.zeros((1, in_channels, 1), requires_grad=True))
...
from util_ldm import instantiate_from_config
